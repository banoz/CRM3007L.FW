...
- **Example**: 950 represents 95.0°C.
- **Safety limit**: `set_coffee_power()` in `board.c` enforces a 1200 (120°C) cutoff via `COFFEE_TEMP_MAX` after PID output is computed, so higher setpoints still stop heating once the limit is reached (PID state is not reset by this safety cutoff).
