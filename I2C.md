...
- **Safety limit**: `set_coffee_power()` enforces a 1200 (120°C) cutoff after PID output is computed, so higher setpoints still stop heating once the limit is reached (PID state is not reset by this safety cutoff).
