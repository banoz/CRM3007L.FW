...
- **Example**: 950 represents 95.0°C; coffee heating is safety-limited to 1200 (120°C) in `set_coffee_power()` after PID output is computed, so higher setpoints still stop heating once the limit is reached (PID state is not reset by this safety cutoff).
